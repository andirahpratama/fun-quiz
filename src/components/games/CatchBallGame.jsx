...
export default function CatchBallGame({
  ...
  const [toastMessage, setToastMessage] = useState('Keranjang Siap! Geser untuk menangkap bola 🏀');
      ...
      if (!isPausedRef.current) {
        ...
        balls.forEach(b => {
          ...
          if (
            ...
            if (b.isQuestion) {
              // PAUSE GAME IMMEDIATELY ON QUESTION CATCH!
  ...
  const renderDigitalCertificate = () => {
    ...
    ctxCert.fillText(`Telah berhasil menuntaskan ${questions.length} tantangan menangkap bola soal interaktif`, w / 2, 370);
  ...
  return (
    <div className="w-full min-h-screen bg-slate-900 text-white font-['Fredoka','Nunito',sans-serif] flex flex-col justify-between">
      ...
      <header className="w-full bg-emerald-900/90 backdrop-blur border-b-4 border-yellow-400 py-2.5 px-4 sticky top-0 z-40 shadow-lg">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-base sm:text-xl font-bold tracking-wide text-yellow-300 leading-tight uppercase">
                {subject || 'TANGKAP BOLA QUEST'}
              </h1>
              <p className="text-xs text-emerald-200 font-semibold tracking-wider">
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            {studentInfo && (
              <div className="hidden sm:flex items-center bg-emerald-950/70 px-3 py-1 rounded-full border border-emerald-400/40 text-xs text-cyan-200">
                👤 <span className="ml-1 font-bold text-white max-w-[120px] truncate">{studentInfo.name}</span>
                <span className="ml-1 bg-yellow-400 text-emerald-900 px-1.5 py-0.2 rounded font-black text-[10px]">{studentInfo.studentClass}</span>
              </div>
            )}
            <div className="flex items-center bg-yellow-500 text-emerald-950 px-3 py-1 rounded-full font-black text-sm shadow-md border-2 border-white">
              ⭐ <span className="ml-1 text-base">{score}</span>
            </div>
            <button
              onClick={toggleSound}
              className="w-9 h-9 rounded-full bg-emerald-700 hover:bg-emerald-600 border-2 border-yellow-300 flex items-center justify-center text-sm shadow transition"
            >
              {soundEnabled ? '🔊' : '🔇'}
            </button>
          </div>
      ...
      {viewState === 'game' && (
        <section className="flex-1 w-full max-w-4xl mx-auto my-3 h-[75vh] min-h-[500px] max-h-[720px] relative bg-emerald-950 rounded-3xl overflow-hidden border-4 border-yellow-400 shadow-2xl flex flex-col">
          <div className="w-full bg-emerald-950/70 backdrop-blur px-4 py-2 flex items-center justify-between text-xs sm:text-sm font-bold border-b border-emerald-400/30 z-20">
            <div className="flex items-center space-x-2 text-cyan-200">
              <span>Target Bola Tersisa:</span>
          ...
          <div className="w-full bg-emerald-950/85 backdrop-blur py-2.5 px-4 flex items-center justify-between border-t-2 border-yellow-400/40 z-20">
            <div className="text-xs text-emerald-200 flex items-center space-x-1.5 font-bold">
      ...
      {activeQuestion && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-3 animate-fadeIn">
          <div className="bg-gradient-to-b from-emerald-800 via-teal-800 to-emerald-950 w-full max-w-2xl rounded-3xl border-4 border-yellow-400 shadow-2xl p-5 sm:p-7 relative max-h-[92vh] flex flex-col justify-between overflow-y-auto text-white">
            
            <div className="flex items-center justify-between border-b-2 border-emerald-400/40 pb-3 mb-3">
            ...
            <div className="my-2 bg-emerald-950/60 p-4 rounded-2xl border border-sky-400/30 shadow-inner">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 my-3">
              {activeQuestion.options.map((optText, optIdx) => {
                let btnStyle = 'bg-emerald-900/70 hover:bg-teal-700/80 border-emerald-400/50';
                ...
                if (hasAnsweredModal) {
                  if (optIdx === activeQuestion.correctAnswer) {
                    ...
                    btnStyle = 'bg-emerald-950/40 border-emerald-900 text-slate-400 opacity-60';
                  }
                }
            ...
            {hasAnsweredModal && (
              <div className="mt-2 pt-2 border-t border-emerald-400/30 flex justify-end">
                <button
      ...
      {viewState === 'result' && (
        <section className="w-full max-w-4xl mx-auto my-auto p-4 sm:p-8 bg-slate-900/90 rounded-3xl border-4 border-yellow-400 shadow-2xl text-center space-y-6 animate-fadeIn">
          ...
          <div className="bg-gradient-to-r from-emerald-700 via-teal-600 to-teal-800 p-6 rounded-2xl border-2 border-yellow-300 shadow-lg text-white relative overflow-hidden">
            ...
            <p className="text-emerald-100 text-sm mt-1">Kamu telah menyelesaikan {questions.length} soal kuis {subject}.</p>
            
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 max-w-2xl mx-auto">
              <div className="bg-emerald-950/70 p-3 rounded-xl border border-emerald-400/40">
                <div className="text-xs text-emerald-200">Total Skor</div>
                <div className="text-3xl font-black text-yellow-300">{score}</div>
              </div>
              <div className="bg-emerald-950/70 p-3 rounded-xl border border-emerald-400/40">
                <div className="text-xs text-emerald-200">Jawaban Benar</div>
                <div className="text-3xl font-black text-emerald-400">
                  {Object.values(userAnswers).filter(a => a.isCorrect).length}
                </div>
              </div>
              <div className="bg-emerald-950/70 p-3 rounded-xl border border-emerald-400/40">
                <div className="text-xs text-emerald-200">Jawaban Salah</div>
                <div className="text-3xl font-black text-rose-400">
                  {questions.length - Object.values(userAnswers).filter(a => a.isCorrect).length}
                </div>
              </div>
              <div className="bg-emerald-950/70 p-3 rounded-xl border border-emerald-400/40">
                <div className="text-xs text-emerald-200">Predikat</div>
          ...
          {showReviewAccordion && (
            <div className="text-left bg-slate-950 p-4 sm:p-6 rounded-2xl border border-emerald-500/40 space-y-4">
              <h3 className="text-lg font-black text-yellow-300 border-b border-emerald-500/30 pb-2">
      ...
      <footer className="w-full bg-emerald-950 py-2 text-center text-xs text-emerald-300 border-t border-emerald-800 mt-4">
